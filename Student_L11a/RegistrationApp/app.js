...
const validateRegistration = (req, res, next) => {
    ...
    next(); // If all validations pass, the next function is called, allowing the request to proceed the next middleware function or route handler.
};
...
app.get('/login', (req, res) => {
    res.render('login', { 
        messages: req.flash('success'), // Retrives success message from the session and pass them to the view
        errors: req.flash('error') // Retrives error message from the session and pass them to the view
    }); 
});
...
app.post('/login', (req, res) => {
    ...
    db.query(sql, [email, password], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            // ******** TODO (ONLY IN L11B): Update to redirect users to /dashboard route upon successful log in ********//
            res.redirect('/dashboard');
        } else {
            // Invalid credentials
